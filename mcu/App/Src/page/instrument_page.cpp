...
{
    this->key_handlers[USER_KEY_1].on_pressed = [this](key_state state) { route_to(&buzzer_menu_page_instance); };
    this->key_handlers[USER_KEY_2].on_pressed = [this](key_state state) { route_to(&buzzer_menu_page_instance); };
    for (uint8_t i = 0; i <16; i++)
    {
        this->key_handlers[KEY_X0Y0 + i].on_pressed = [this, i](key_state state) {
            buzzer_start_play(single_tone + i, 1);
        };
        this->key_handlers[KEY_X0Y0 + i].on_released = [this](key_state state) { buzzer_start_play(nullptr, 0); };
    }
}
